document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const validatableSelectors = 'input:not([type="submit"]):not([type="reset"]):not([type="button"]):not([type="hidden"]):not([type="checkbox"]):not([type="radio"]), textarea, select';

    contactForm.addEventListener('blur', function (e) {
      const field = e.target.closest(validatableSelectors);
      if (field && contactForm.contains(field)) {
        validateField(field);
      }
    }, true);
    ...
    contactForm.addEventListener('input', function (e) {
      const field = e.target.closest(validatableSelectors);
      if (field && contactForm.contains(field) && field.classList.contains('field-error')) {
        validateField(field);
      }
    });

    contactForm.addEventListener('submit', function (e) {
      const fields = contactForm.querySelectorAll(validatableSelectors);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "יותם ישראל - ביטוח ופיננסים משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'c2aefc02-a972-4095-9a6b-5a7493d45d93',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'c2aefc02-a972-4095-9a6b-5a7493d45d93';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'c2aefc02-a972-4095-9a6b-5a7493d45d93';
